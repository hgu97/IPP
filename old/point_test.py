...
og_width = 800
og_height = 600
og_color = cv.resize(og_color, (og_width, og_height),
                     interpolation=cv.INTER_AREA)
burr_color = cv.resize(burr_color, (og_width, og_height),
                       interpolation=cv.INTER_AREA)
...
# %% Template matching
cv.imshow('a', og_trim)
burr_left, burr_upper, burr_right, burr_lower = deburr.TempMatch(
    og_trim_g, burr)
...
matches, matchesMask, kp1, kp2, src, dst = deburr.F_match(
    og_trim_g, burr_trim_g)
...
good_matches, knn_matchesMask, kp1, kp2, knn_src, knn_dst = deburr.F_knnmatch(
    og_trim_g, burr_trim_g)
...
Home = RDK.Item('Target1')
target_pose = target.Pose()
xyz_ref = target_pose.Pos()

robot.setSpeed(10, 5, -1, -1)
robot.MoveJ(Home)
...
#     if tool_feed[i]>=5:
#         robot.setSpeed(1,-1,-1,-1)
#     elif tool_feed[i]>=4:
#         robot.setSpeed(1,-1,-1,-1)
#     elif tool_feed[i]>=3:
#         robot.setSpeed(1,-1,-1,-1)
#     elif tool_feed[i]>=2:
#         robot.setSpeed(500,-1,-1,-1)
#     elif tool_feed[i]>=1:
#         robot.setSpeed(500,-1,-1,-1)

i= 0
# Calculate the new position around the reference:
x = int(og_largestcnt[i, :, 0])+xyz_ref[0]  # new X coordinate
y = int(og_largestcnt[i, :, 1])+xyz_ref[1]  # new Y coordinate
